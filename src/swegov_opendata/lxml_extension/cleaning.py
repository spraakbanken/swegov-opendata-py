...
def clean_html(text):
    ...
    text = re.sub(
        r"&#(" + "|".join(str(i) for i in [*range(10), *range(11, 32), *range(127, 160)]) + ");",
        "",
        text,
    )
    ...
    text = re.sub(r"<!--\[if lte IE 7\]>", "", text)
    text = re.sub(r"<!--\[if gte IE 8\]>", "", text)
    text = re.sub(r"<!\[if \!vml\]>", "", text)
    text = re.sub(r"<!\[if \!supportMisalignedColumns\]>", "", text)
    text = re.sub(r"<!\[if \!supportLineBreakNewLine\]>", "", text)
    text = re.sub(r"<!\[endif\]-->", "", text)
    text = re.sub(r"<!\[endif\]>", "", text)
    text = re.sub(r"<!\[if \!supportEmptyParas\]>", "", text)
